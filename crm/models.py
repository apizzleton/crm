...
class Property(db.Model):
    ...
    tasks = db.relationship('Task', back_populates='related_property', cascade='all, delete-orphan')
    
    def __repr__(self):
        return f'<Property {self.name or self.address}>'


class PropertyOwner(db.Model):
    """Junction table linking properties to contacts (owners)."""
    __tablename__ = 'property_owners'
    
    id = db.Column(db.Integer, primary_key=True)
    property_id = db.Column(db.Integer, db.ForeignKey('properties.id'), nullable=False)
    contact_id = db.Column(db.Integer, db.ForeignKey('contacts.id'), nullable=False)
    ownership_percentage = db.Column(db.Numeric(5, 2))  # Optional ownership percentage
    notes = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Task(db.Model):
    ...
    related_property = db.relationship('Property', back_populates='tasks')
